...
Mat videoProcessor::videoProcess(Mat Input) {
    ...
    int w = output.cols;
    int h = output.rows;
    srcQuad[0] = Point2f(420, 364);
    srcQuad[1] = Point2f(565, 364);
    srcQuad[2] = Point2f(800, 509);
    srcQuad[3] = Point2f(200, 509);
    Point2f dstQuad[4];
    dstQuad[0] = Point2f(0.0 , 0.0);
    dstQuad[1] = Point2f(w , 0.0);
    dstQuad[2] = Point2f(w, h);
    dstQuad[3] = Point2f(0, h);
    Mat transform4p = perspectiveTransformImage(output, srcQuad, dstQuad);
    //imageShow("output_i", output);
    //imageShow("transform4p", transform4p);
    Mat HSV = convertColor(transform4p, CV_BGR2HSV);
    ...
    Mat roadCanny = cannyEdge(gray_th, 50, 100);
    //imageShow("roadCanny", roadCanny);
    vector<Vec4i> lines = HoughLinesP(roadCanny, 1.0, CV_PI/75.0, 25, 5, 500);
    Mat roadLines = drawLanes(roadCanny, lines);
    //imageShow("roadLines", roadLines);
    Mat returnform4p = perspectiveTransformImage(roadLines, dstQuad, srcQuad);
    output = weightedSum(returnform4p, output);
    return output;
}
int main(void) {
    videoProcessor processor;
    processor.setInput(path+roadVideo);
    processor.setOutput(path+"Lane_Detection_Videos/DrawLane_Video_Project.mp4");
...
Mat drawLanes(Mat image, vector<Vec4i> lines) {
    ...
    Point left_xp = MedianVector(left_x);
    Point right_xp = MedianVector(right_x);
    Point left_yp = MedianVector(left_y);
    Point right_yp = MedianVector(right_y);

    int min_y = 0;//image.rows * 0.6;
    ...
    line( result, left_min, left_max, Scalar(0,0,255), 10, CV_AA);
    line( result, right_min, right_max, Scalar(0,0,255), 10, CV_AA);
